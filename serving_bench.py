...
def warm_up(engine, args):
    ...
    prompts = [
        [randint(0, 10000) for _ in range(args.random_input_len)] for _ in range(50)
    ]
    ...
    sampling_params = [
        SamplingParams(
            temperature=0.6, ignore_eos=True, max_tokens=args.random_output_len
        )
...
def main():
            ...
            if not engine.is_finished():
                ...
                for seq_id, output_ids in finished_outputs:
                    if seq_id in metrics:
                        metrics[
                            seq_id
                        ].record_first_token()  # Ensure first token time is recorded
    ...
    total_output_tokens = sum(
        m.output_len for m in metrics.values() if m.output_len != -1
    )
    ...
    avg_latency = np.mean(
        [m.latency for m in metrics.values() if m.completion_time != -1]
    )
